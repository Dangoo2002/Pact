...
export async function GET() {
  try {
    ...
    if (gapHeatmapResult.rows.length > 0) {
      const topGap = gapHeatmapResult.rows[0];
      recommendations.push(`Schedule review session on ${topGap.concept} (${Math.round(topGap.struggling_percentage)}% struggling)`);
      recommendations.push(`Create targeted practice quiz for ${topGap.concept}`);
      recommendations.push(`Provide additional examples and resources`);
    } else {
