...
import { 
  ...
  BookOpen, Target, LayoutDashboard, GraduationCap
} from 'lucide-react';
...
export default function RecommendationsPage() {
  ...
  useEffect(() => {
    ...
    fetchRecommendations(studentId, 20)
      .then(data => {
      ...
      <div className="md:ml-64">
        ...
        <div className="p-4 md:p-6">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-400 mt-1">Learning resources tailored to your knowledge gaps</p>
          </div>

          {recommendations.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {recommendations.map((rec, idx) => {
                const Icon = typeIcons[rec.resource_type] || FileText;
                const colorClass = typeColors[rec.resource_type] || 'bg-gray-500/20 text-gray-400';
                  ...
                  <div key={idx} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4 hover:border-blue-500/30 transition">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 capitalize">{rec.resource_type}</p>
                      </div>
                      {rec.score && (
                    ...
                    <p className="text-sm text-gray-400 mb-4">{rec.reason || 'Recommended for your learning gaps'}</p>
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/10">
                      <a
                        href={rec.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={() => handleResourceClick(rec.resource_id)}
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl text-center py-12">
              ...
              <p className="text-sm text-gray-500 mb-4">Complete more quizzes to get personalized recommendations</p>
