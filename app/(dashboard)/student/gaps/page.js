...
import { 
  ...
  Menu, User, LogOut, Bell, Brain, Target, BookOpen
} from 'lucide-react';
...
export default function StudentGapsPage() {
  ...
  useEffect(() => {
    ...
    fetchGapProfile(session.user.id).then(data => {
      ...
      <div className="md:ml-64">
        ...
        <div className="p-4 md:p-6">
          <div className="flex justify-between items-center mb-6">
            <div><h1 className="text-2xl md:text-3xl font-bold text-white">Knowledge Gaps</h1><p className="text-sm text-gray-400 mt-1">Identified areas where you need improvement</p></div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
              <div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-red-500/20"><AlertCircle size={20} className="text-red-400" /></div><div><p className="text-2xl font-bold text-white">{primaryGaps.length}</p><p className="text-sm text-gray-500">High Priority Gaps</p></div></div>
            </div>
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
              <div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-yellow-500/20"><TrendingDown size={20} className="text-yellow-400" /></div><div><p className="text-2xl font-bold text-white">{secondaryGaps.length}</p><p className="text-sm text-gray-500">Improvement Areas</p></div></div>
            </div>
          </div>
                ...
                {primaryGaps.map((gap, idx) => (
                  <div key={idx} className="border-b border-white/10 pb-4 last:border-0 last:pb-0">
                    <div className="flex justify-between items-start mb-2">
                      <div><p className="font-medium text-white">{gap.concept} ({gap.language})</p><p className="text-sm text-gray-500">Mastery: {(gap.mastery_score * 100).toFixed(0)}%</p></div>
                    ...
                    {aiExplanation[gap.concept] && <div className="mt-3 p-3 rounded-lg bg-purple-500/10 border border-purple-500/20"><p className="text-sm text-gray-300">{aiExplanation[gap.concept]}</p></div>}
                  </div>
                ))}
              </div>
            </div>
