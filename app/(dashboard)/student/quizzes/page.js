...
const LANGUAGES = ['python', 'java', 'javascript', 'csharp', 'cpp', 'go', 'rust'];
const CONCEPTS = ['variables', 'loops', 'functions', 'arrays', 'classes', 'recursion', 'inheritance', 'polymorphism'];
...
export default function QuizzesPage() {
  ...
  const fetchQuizzes = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/student/quizzes?studentId=${session.user.id}&language=${languageFilter}&concept=${conceptFilter}`);
  ...
  const getAiRecommendation = async () => {
    try {
      const response = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({ 
          message: "Based on typical programming learning paths, recommend which topic I should practice next. Suggest one specific topic and language.",
      ...
      setAiRecommendation("Try Python loops to strengthen your fundamentals.");
    }
  };
        ...
        <div className="p-4 md:p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">AI-generated adaptive assessments</p>
            </div>
          ...
          {aiRecommendation && (
            <div className="bg-gradient-to-r from-purple-500/20 to-blue-500/20 border border-purple-500/30 rounded-xl p-4 mb-6 flex items-center gap-3">
              ...
              <div className="flex-1"><p className="text-xs text-purple-400 mb-1">AI RECOMMENDATION</p><p className="text-sm text-white">{aiRecommendation}</p></div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredQuizzes.map((quiz) => (
              <div key={quiz.id} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4 hover:border-blue-500/30 transition cursor-pointer" onClick={() => startQuiz(quiz.title, quiz.language)}>
                <div className="flex items-start justify-between mb-3"><div className="p-2 rounded-lg bg-blue-500/20"><Code size={18} className="text-blue-400" /></div>{quiz.progress > 0 && (<span className="text-xs px-2 py-1 rounded-full bg-green-500/20 text-green-400">{quiz.progress}% complete</span>)}</div>
                <h3 className="font-semibold text-white mb-1">{quiz.title}</h3>
                <div className="flex items-center gap-2 text-xs mb-3"><span className="px-2 py-0.5 rounded-full bg-white/10 text-gray-300 capitalize">{quiz.language}</span><span className={`px-2 py-0.5 rounded-full ${quiz.difficulty === 'beginner' ? 'bg-green-500/20 text-green-400' : quiz.difficulty === 'intermediate' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-red-500/20 text-red-400'}`}>{quiz.difficulty}</span></div>
                <div className="flex items-center gap-3 text-xs text-gray-500 mb-4"><div className="flex items-center gap-1"><Clock size={12} /><span>{quiz.estimated_time} min</span></div><div className="flex items-center gap-1"><BookOpen size={12} /><span>{quiz.question_count} questions</span></div></div>
                <button className="w-full py-2 rounded-lg bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 transition text-sm font-medium">Start Quiz</button>
              </div>
            ))}
          </div>
